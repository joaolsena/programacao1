...
void tela() {
    ...
    for (int i = 0; i < MAX_tela_y; i++) {
        ...
        for (int j = 0; j < MAX_tela_X; j++) {
            ...
            {
                printf("%c", monstro_3);
            } else if (imagem[i][j] == barreira_forma_1 || imagem[i][j] == barreira_forma_2) {
                printf("%c", imagem[i][j]);
...
void inicia_barreira() {
    for (int i = 0; i < max_barreira; i++) {
        barreiras[i].ativo = 1;
        barreiras[i].x = (posicao-2) +  i;
        barreiras[i].y = MAX_tela_y - 5;
        barreiras[i].resistencia = 5;
        if (barreiras[i].x == MAX_tela_X/2 || barreiras[i].x == MAX_tela_X/4 || barreiras[i].x == MAX_tela_X || barreiras[i].x == (MAX_tela_X/2)+10 )
        {
             barreiras[i].ativo = 0;
            imagem[barreiras[i].y ][barreiras[i].x ] = ' ';
        }else
        imagem[barreiras[i].y ][barreiras[i].x ] = barreira_forma_1;
