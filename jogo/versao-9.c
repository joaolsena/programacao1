...
void inicia_barreira() {
    int grupos = 4; 
    int barreiras_por_grupo = 6; 
    int espaco_entre_grupos = 4; 

    
    int inicio_grupo = (MAX_tela_X - (grupos * barreiras_por_grupo) - ((grupos - 1) * espaco_entre_grupos)) / 2;

    int index_barreira = 0;

    for (int g = 0; g < grupos; g++) {
        for (int i = 0; i < barreiras_por_grupo; i++) {
            barreiras[index_barreira].ativo = 1;
            barreiras[index_barreira].x = inicio_grupo + (g * (barreiras_por_grupo + espaco_entre_grupos)) + i;
            barreiras[index_barreira].y = MAX_tela_y - 5; 
            barreiras[index_barreira].resistencia = 10;
            
            imagem[barreiras[index_barreira].y][barreiras[index_barreira].x] = barreira_forma_1;
...
void movimento_monstro() {
    ...
    if (tempo_monstro >= (3 - velocidade)) {
        ...
        for (int i = 0; i < MAX_monstro * 2; i++) {
            if (monstro[i].ativo){
            if(monstro[i].x <= 0 || monstro[i].x >= MAX_tela_X -1) {
                direcao *= -1;
